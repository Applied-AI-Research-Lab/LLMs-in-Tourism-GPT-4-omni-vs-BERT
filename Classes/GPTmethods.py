...
class GPTmethods:
            ...
            data.append(
                {
                    "messages": [
                        ...
                        {
                            "role": "user",
                            "content": f"Predict the star rating (integer between 1 and 5) to the following review. Return your response in JSON format like this example {{'rating': integer}}. Please avoid providing additional explanations. Review:\n{row['Review']}"
                        },
                        {
                            "role": "assistant",
    ...
    def gpt_prediction(self, input):
        ...
        return self.clean_response(response=content, a_field=input['Review'])
        ...
        for index, row in df.iterrows():
            ...
            if pd.isnull(row[prediction_column]):
                ...
                if not prediction['status']:
                    ...
                    if prediction['data']['rating'] != '':  # TODO! Change it!
                        # Update the DataFrame with the evaluation result
                        # df.at[index, prediction_column] = prediction['data']['rating']  # TODO! Change it!
                        # for integers only
                        ...
                        logging.error(
                            f"No rating instance was found within the data for '{row['Review']}', and the "
                            f"corresponding prediction response was: {prediction}.")  # TODO! Change it!
                        return {"status": False,
                                "data": f"No rating instance was found within the data for '{row['Review']}', "
...
# GPT_FT = GPTmethods(model_id='ft:gpt-4o-2024-08-06:personal:hotels-4:AG99JwMv')
...
# Make predictions after Fine-tuning using the Fine-tuned (model_id)
# GPT_FT = GPTmethods(model_id='ft:gpt-4o-mini-2024-07-18:personal:hotels-mini:AGNG8CSI')
